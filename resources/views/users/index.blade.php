...
@section('content')
    ...
    <form id="bulkForm" method="POST" action="{{ route('users.bulk-destroy') }}" data-confirm data-confirm-title="Delete selected users?">
        ...
        >
            ...
            @forelse ($users as $user)
                <tr>
                    <td>
                        @if (! $user->trashed())
                            <input type="checkbox" name="ids[]" form="bulkForm" value="{{ $user->id }}" class="form-check-input row-checkbox">
                        @endif
                    </td>
        ...
        @can('users.delete')
            <div class="mt-2">
                <button type="submit" form="bulkForm" class="btn btn-outline-danger btn-sm">
