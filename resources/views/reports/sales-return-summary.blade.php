...
@section('title', 'Sales Return Summary Report')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('reports.index') }}">Reports</a></li>
    <li class="breadcrumb-item active">Sales Return Summary</li>
@endsection

@section('content')
    <x-filter-bar :action="route('reports.sales-return-summary')">
        <div class="col-md-3">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h6 class="mb-0">Sales Return Summary</h6>
            <div class="btn-group btn-group-sm">
                <a href="{{ route('reports.sales-return-summary.export', request()->query()) }}" class="btn btn-outline-secondary"><i class="ti ti-file-spreadsheet me-1"></i>Export</a>
                <a href="{{ route('reports.sales-return-summary.print', request()->query()) }}" class="btn btn-outline-secondary" target="_blank"><i class="ti ti-printer me-1"></i>Print</a>
            </div>
        </div>
                ...
                <tbody>
                    @forelse ($rows as $row)
                        <tr>
                            ...
                            <td class="fw-semibold">{{ number_format($row->total_credited, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="text-center text-muted py-4">No return data for this period.</td>
                        </tr>
                    @endforelse
                </tbody>
                @if ($rows->isNotEmpty())
                    <tfoot>
                        <tr>
                            ...
                            <td class="fw-semibold">{{ $totals['returns_count'] }}</td>
                            <td class="fw-semibold">{{ $totals['pending_count'] }}</td>
                            <td class="fw-semibold">{{ $totals['rejected_count'] }}</td>
                            <td class="fw-semibold">{{ $totals['received_count'] }}</td>
                            <td class="fw-semibold">{{ number_format($totals['total_credited'], 2) }}</td>
                        </tr>
                    </tfoot>
                @endif
            </table>
