...
@section('title', 'Achievement Summary Report')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('reports.index') }}">Reports</a></li>
    <li class="breadcrumb-item active">Achievement Summary</li>
@endsection

@section('content')
    <x-filter-bar :action="route('reports.achievement-summary')">
        <div class="col-md-3">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h6 class="mb-0">Achievement Summary</h6>
            <div class="btn-group btn-group-sm">
                <a href="{{ route('reports.achievement-summary.export', request()->query()) }}" class="btn btn-outline-secondary"><i class="ti ti-file-spreadsheet me-1"></i>Export</a>
                <a href="{{ route('reports.achievement-summary.print', request()->query()) }}" class="btn btn-outline-secondary" target="_blank"><i class="ti ti-printer me-1"></i>Print</a>
            </div>
        </div>
        <div class="text-muted small px-3 pt-3">Approved entries only.</div>
                ...
                <tbody>
                    @forelse ($rows as $row)
                        <tr>
                            ...
                            <td class="fw-semibold">{{ number_format($row->total_order_achieved, 2) }}</td>
                            <td>{{ number_format($row->total_collection_achieved, 2) }}</td>
                            <td>{{ $row->total_quantity_achieved }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4">No approved achievement entries for this period.</td>
                        </tr>
                    @endforelse
                </tbody>
                @if ($rows->isNotEmpty())
                    <tfoot>
                        <tr>
                            ...
                            <td class="fw-semibold">{{ $totals['entries_count'] }}</td>
                            <td class="fw-semibold">{{ number_format($totals['total_order_achieved'], 2) }}</td>
                            <td class="fw-semibold">{{ number_format($totals['total_collection_achieved'], 2) }}</td>
                            <td class="fw-semibold">{{ $totals['total_quantity_achieved'] }}</td>
                        </tr>
                    </tfoot>
                @endif
            </table>
