...
class StoreAchievementEntryRequest extends FormRequest
{
    ...
    {
        return $this->user()?->can('create', AchievementEntry::class) ?? false;
    }

    /**
     * Defaults `mode` when the caller doesn't send one at all — mirrors
     * StoreTargetRequest's own rule.
     *
    ...
    public function rules(): array
    {
        return [
            'user_id' => [
                ...
                Rule::unique('achievement_entries', 'user_id')->where(
                    fn ($query) => $query->where('entry_date', $this->input('entry_date'))
                ),
            ],
